...
def get_versions(charts_dir,env, repo):
   ...
   # Initialise a dictionary to store service versions
   versions = {}
...
def options():
    ...
    # Add a source environment flag for the prod push, e.g., dev, staging
    parser.add_argument('--source-env', help='Select environment')

    # Add a target environment flag for the prod push, e.g., prod
...
def update_versions(env,versions, repo, branch):
    ...
    for service in services:
        ...
        # Create a path for the fi e.g. envs/prod/payments/application.yaml
...
def get_repo(name):
    ...
    # Get the personal authorisation token from an environment variable, e.g. export GITHUB_TOKEN=github_pat_123
    github_token = os.environ['GITHUB_TOKEN']

    # Create authorisation based on the token
    auth = Auth.Token(github_token)

    # Autorize with GitHub using the token
    g = Github(auth=auth)
...
def main():
    ...
    repository = get_repo('ditvtvtvt/k8s')
    ...
    if args.action == 'push':
        ...
        # Create a new branch name for the PullRequest, e.g. prod-push-20240729
        ...
        latest_version = get_versions('helm-charts', args.source_env, repository)

        # Update versions in the specified environment to the latest ones
        update_versions(args.target_env, latest_version, repository, new_branch)

        # Create Pull Request to enable Continuous delivery
